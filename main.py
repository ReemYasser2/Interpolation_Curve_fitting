...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def cubic(self):
        ...
        # Plotting the Graph
        time =np.linspace(self.time_array.min(), self.time_array.max(), 500)
     
        magnitude = make_interp_spline(self.time_array , self.magnitude_array)(time)
        self.equation()
        self.plot_widget.clear()
        self.plot_widget.plot(self.time, self.magnitude) 

        self.plot_widget.plot(time, magnitude, pen='y')
    ...
    def extrapolation(self):
        ...
        if int(self.interpolation_type.currentIndex()) == 0:
            ...
            # Plotting the Graph
            time =np.linspace(self.time_array.min(), self.time_array.max(), 500)[0:int(len(self.time) * percent_data / 100)]
            magnitude = make_interp_spline(self.time_array , self.magnitude_array)(time)[0:int(len(self.time) * percent_data / 100)]
            time_predict = (self.time_array.min(), self.time_array.max(), 500)[int(len(self.time) * percent_data / 100):(len(self.time))]
            coeff = np.polyfit(time, magnitude, 3)
            prediction = np.polyval(coeff, time_predict)
            arr_predict = np.array(prediction)
            self.plot_widget.plot(time, magnitude, pen='y')
            ...
            Y_ = make_interp_spline(self.time_array , self.magnitude_array, k= deg)(X_)
            time_predict = (self.time_array.min(), self.time_array.max(), 500)[int(len(self.time) * percent_data / 100):(len(self.time))]
            coeff = np.polyfit(X_, Y_, deg)
            prediction = np.polyval(coeff, time_predict)
            arr_predict = np.array(prediction)

            self.plot_widget.plot(X_[0:int(len(self.time) * percent_data / 100)], Y_[0:int(len(self.time) * percent_data / 100)], pen='b')
